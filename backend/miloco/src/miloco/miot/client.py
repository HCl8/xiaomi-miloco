...
class MiotProxy:
    ...
    async def reset_central_identity_async(self) -> None:
        """On logout / account switch: drop the local central-hub identity so the
        next account starts clean — delete the current account's client cert and
        regenerate the virtual did (persisted to KV). The client rebuilt by
        init() injects the new did; the new account then signs a new cert. Cloud
        identity (device uuid) is untouched.
        """
