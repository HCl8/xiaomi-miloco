...
from an *ephemeral* UDP source port to ``224.0.0.251:5353`` and responders
reply **unicast** back to that port. Because we never bind port 5353 we don't
contend with a system mDNS responder — notably macOS ``mDNSResponder`` owns
5353 and starves any second listener (python-zeroconf receives nothing there,
verified on-device), whereas this pure-stdlib socket approach works on both
Linux and macOS with one code path and no subprocess.

A single PTR query returns the whole record set (PTR + SRV + TXT + A in the
additional section), so one round-trip per gateway yields instance / host /
port / IPv4 / profile with no follow-up lookups. Discovery is poll-based
(periodic re-query); service *removals* are ignored — the local MQTT
connection closes on its own (matches the Xiaomi Home integration behavior).
"""
