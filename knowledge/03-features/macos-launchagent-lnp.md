...
| 保活       | `autorestart=true` + `startretries=3`                                | `KeepAlive={Crashed:true}`（仅崩溃重拉、clean exit 不拉；launchd ~10s 节流）                              |
...
- launchd 无 supervisord 的 unix socket 控制通道；`startretries=3→FATAL` 换成 `KeepAlive={Crashed:true}` + launchd ~10s 节流。launchd 本身无"放弃"态,但 CLI 侧 `service start/restart` 会在健康探测窗口内检测 crashloop(反复换 pid ≥3 次)并 `bootout` 停掉、报失败,补上"放弃"语义。
