...
def _wait_for_health(
    ...
    （supervisord 传 FATAL 检测；launchd 无对应态时传 None）。"""
    import httpx
...
def _generate_launchagent_plist(cmd: list[str]) -> None:
    ...
    plist = {
        ...
        "RunAtLoad": True,
        # 顶替 supervisord autorestart:仅崩溃(异常退出)时重启,clean exit 不重启
        # (bootout 是移除 job、不触发)。launchd 内建 ~10s ThrottleInterval 防紧循环
        # ——对齐 supervisord「autorestart + startsecs」的意图,又不像裸 True 那样连
        # 正常退出也拉起。
...
def _launchd_crashloop_check() -> Callable[[], bool]:
    ...
    launchd `KeepAlive={Crashed:true}` 没有 supervisord `startretries→FATAL` 的
...
def _launchd_start(cfg: dict, pretty: bool) -> None:
    ...
    # 兜底：supervisord 被 SIGKILL 后其子 backend 可能仍占端口，按端口定位并清理
